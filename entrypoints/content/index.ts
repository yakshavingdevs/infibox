...
import { addTodo, loadTodoTasks } from "./todo";
...
export default defineContentScript({
  ...
  main() {
    chrome.storage.local.get({ primaryColor: "#000000" }, (settings: Record<string, unknown>) => {
      (shadow.host as HTMLElement).style.setProperty("--primary", settings.primaryColor as string);
    });

    chrome.storage.onChanged.addListener((changes, area) => {
      if (area === "local" && "primaryColor" in changes) {
        (shadow.host as HTMLElement).style.setProperty("--primary", changes.primaryColor.newValue as string);
      }
    });
...
export function initCmdk(): void {
  ...
  globalState.activeIndex = 0;
  globalState.currentFiltered = globalState.currentList as unknown as SearchResult[];
  setRenderMode("list");
}

export function showCmdk(toolCommand: Command | null = null): void {
...
export function hideCmdk(): void {
  shadow.getElementById("cmdk-overlay")!.classList.remove("open");
  resetCmdkState();
  document.body.focus();
}

export const contextCommandMap: Record<string, string> = {
...
export function searchCommandByPath(path: string, list: Command[]): Command | null {
