...
export function executeCommand(result: SearchResult): void {
  if (!result?.cmd) {
...
export function globalSearch(query: string, list: Command[], breadcrumb: Command[]): SearchResult[] {
...
export function updateListItems(listEl: HTMLElement, results: SearchResult[]): void {
  listEl.innerHTML = "";
  if (results.length === 0) {
...
export function handleListInputKeyDown(e: KeyboardEvent): void {
  const inputEl = shadow.getElementById("cmdk-input") as HTMLInputElement | null;
...
export function renderListMode(filter = ""): void {
  ...
  let results: SearchResult[] = filter.trim() === ""
  ...
  inputEl.addEventListener("input", (e) => {
    ...
    if (!target) return;
    globalState.activeIndex = 0;
    results = target.value.trim() === ""
      ? globalState.currentList.map((cmd) => ({ cmd, breadcrumb: [] }))
      : globalSearch(target.value, defaultCommands, []);
    globalState.currentFiltered = results;
    updateListItems(listEl, results);
  });
}
